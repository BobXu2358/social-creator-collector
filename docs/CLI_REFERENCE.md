...
| `douyin video-detail` | `ok`, `json`, `markdown`, `aweme_id`, `completion_rate_pct`, `avg_watch_duration_s`, `traffic_sources`; `partial`, `warning` when item metrics are unavailable but other detail data exists | A completely empty response still fails. |
...
| Douyin video-detail | `schema_version`, `account`, `platform`, `source`, `captured_at`, `aweme_id`, `endpoints_seen`, `field_notes`, `video`; `partial`, `warning`, `diagnostics` when item metrics are unavailable but other detail data exists | `video` is a `video_row`; partial diagnostics use `reason=item_compare_metrics_unavailable` and list preserved `available_data`. |
