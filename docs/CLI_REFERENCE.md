...
| `douyin video-detail` | `ok`, `json`, `markdown`, `aweme_id`, `completion_rate_pct`, `avg_watch_duration_s`, `traffic_sources`; `partial`, `warning` when item metrics are unavailable but identified detail data exists | `ok=true, partial=true` does not guarantee every video metric is present; unavailable metrics are omitted. A response without work identity still fails. |
